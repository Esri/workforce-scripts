...
def main(arguments):
		...
		for dispatcher in existing_dispatchers:
			...
			if dispatcher.user_id and dispatcher.user_id != arguments.username:
				...
				if not dispatcher.user_id:
					logger.info("Dispatcher was skipped from migrating. The dispatcher does not a valid user_id in the layer, or 2. The dispatcher was already added. Please check the original dispatchers layer.")
	...
	for worker in existing_workers:
		if worker.attributes[project._worker_schema.user_id]:
			worker_name = worker.attributes[project._worker_schema.user_id] if worker.attributes[project._worker_schema.name] is None else worker.attributes[
	...
	for assignment in existing_assignments:
		if assignment.attributes[project._assignment_schema.assignment_type]:
			assignment_location = (str(assignment.geometry["x"]) + " " + str(assignment.geometry["y"])) if assignment.attributes[project._assignment_schema.location] is None else \
			...
			attributes = {v2_project._assignment_schema.status: assignment_status,
								...
								v2_project._assignment_schema.assignment_type: get_assignment_type_global_id(new_assignment_types, assignment_type_name),
								...
								v2_project._assignment_schema.worker_id: get_worker_global_id(project.workers.search(), assignment.attributes[project._assignment_schema.worker_id]),
								...
								v2_project._assignment_schema.dispatcher_id: get_dispatcher_global_id(arguments.skip_dispatchers, project.dispatchers.search(),assignment.attributes[project._assignment_schema.dispatcher_id]),
	...
	if len(project.assignments_layer.attachments.search("1=1")) == len(v2_project.assignments_layer.attachments.search("1=1")):
